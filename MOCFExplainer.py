...
class Explainer():
    def __init__(self,
        ...
        self.costFunction = self.constructCostFunction()

    def constructCostFunction(self):

        print('Constructing cost function according to soundCF and feasibleAR hyper-parameters ...')
        ...
        if self.feasibleAR == False and self.soundCF == False:
            ...
            def costFunction(x_ord, x_org, cf_class, cf_range, probability_thresh, proximity_model, connectedness_model,
                             user_preferences, dataset, predict_fn, predict_proba_fn, feature_width, continuous_indices,
                             discrete_indices, eaScaler, correlationModel, cf_theta):

                # constructing counter-factual from the EA decision variables
                cf_theta = np.asarray(cf_theta)
                cf_org = theta2org(cf_theta, eaScaler, dataset)
                cf_ord = org2ord(cf_org, dataset)
                cf_ohe = ord2ohe(cf_ord, dataset)

                # objective 1: Prediction Distance
                prediction = predictionDistance(cf_ohe, predict_fn, predict_proba_fn,
                                                probability_thresh, cf_class, cf_range)
                # objective 2: Feature Distance
                distance = featureDistance(x_ord, cf_ord, feature_width, continuous_indices, discrete_indices)

                # objective 3: Sparsity
                sparsity = Sparsity(x_org, cf_org)

                return prediction, distance, sparsity
            ...
            def costFunction(x_ord, x_org, cf_class, cf_range, probability_thresh, proximity_model, connectedness_model,
                ...
                proximity = Proximity(cf_theta, proximity_model)

                ## objective 5: Connectedness
                connectedness = Connectedness(cf_theta, connectedness_model)

                return prediction, distance, sparsity, proximity, connectedness
            ...
            def costFunction(x_ord, x_org, cf_class, cf_range, probability_thresh, proximity_model, connectedness_model,
                ...
                actionable = actionableRecourse(x_org, cf_org, user_preferences)

                ## objective 5: Correlation
                correlation = Correlation(x_ord, cf_ord, cf_theta, feature_width,
                                          continuous_indices, discrete_indices, correlationModel)

                return prediction, distance, sparsity, actionable, correlation
            ...
            def costFunction(x_ord, x_org, cf_class, cf_range, probability_thresh, proximity_model, connectedness_model,
                ...
                return prediction, distance, sparsity, proximity, connectedness, actionable, correlation
    ...
    def proximityModel(self):
        ...
        for key, data in self.groundtruth_data.items():
            ...
            data_theta = self.eaScaler.transform(data)
    ...
    def connectednessModel(self):
        ...
        for key, data in self.groundtruth_data.items():
            data_theta = self.eaScaler.transform(data)
            dist = pairwise_distances(data_theta, metric='minkowski')
            ...
            hdbscan_model = hdbscan.HDBSCAN(min_samples=2, cluster_selection_epsilon=float(epsilon),
                                            metric='minkowski', p=2, prediction_data=True).fit(data_theta)
    ...
    def correlationModel(self):
        ...
        X_train_theta = self.eaScaler.transform(self.X_train)
        correlation_models = []
    ...
    def setupToolbox(self, x_ord, x_org, x_theta, cf_class, cf_range, probability_thresh, user_preferences,
        ...
        toolbox.register("evaluate", self.costFunction, x_ord, x_org, cf_class, cf_range, probability_thresh,
                         ...
                         self.eaScaler, self.correlationModel)
    ...
    def explain(self,
        ...
        x_theta = ord2theta(x, self.eaScaler)
        ...
        neighbor_theta = self.eaScaler.transform(neighbor_data)
        ...
        cf_org = theta2org(cfs_theta, self.eaScaler, self.dataset)
        cf_ord = org2ord(cf_org, self.dataset)
        cfs_ord = pd.DataFrame(data=cf_ord, columns=self.feature_names)

        ## evaluating counter-factuals
        cfs_ord, cfs_eval = evaluateCounterfactuals(cfs_ord, self.dataset, self.predict_fn,
        ...
        x_org, cfs_org, x_cfs_org, x_cfs_highlight = recoverOriginals(x_ord, cfs_ord, self.dataset)
