...
def main():
    ...
    for dataset_kw in datsets_list:
        ...
        for blackbox_name, blackbox_constructor in blackbox_list.items():
            ...
            exists = os.path.isfile(experiment_path + 'benchmark_sound_feasible_%s_cfs_%s_%s.csv'%(dataset['name'], N, n_diversity))
            if exists:
                os.remove(experiment_path + 'benchmark_sound_feasible_%s_cfs_%s_%s.csv'%(dataset['name'], N, n_diversity))
            cfs_results_csv = open(experiment_path + 'benchmark_sound_feasible_%s_cfs_%s_%s.csv'%(dataset['name'], N, n_diversity), 'a')
            ...
            # creating/opening a csv file for storing results
            exists = os.path.isfile(experiment_path + 'benchmark_sound_feasible_%s_eval_%s_%s.csv'%(dataset['name'], N, n_diversity))
            if exists:
                os.remove(experiment_path + 'benchmark_sound_feasible_%s_eval_%s_%s.csv'%(dataset['name'], N, n_diversity))
            eval_results_csv = open(experiment_path + 'benchmark_sound_feasible_%s_eval_%s_%s.csv'%(dataset['name'], N, n_diversity), 'a')

            header = '%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n' % \
                      ('MOCF', '', '', '', '', '', '', '',
                       'DiCE', '', '', '', '', '', '', '')
            eval_results_csv.write(header)

            header = '%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n' % \
                      ('prediction', 'proximity', 'connectedness', 'actionable', 'correlation', 'distance', 'sparsity', 'diversity',
                       'prediction', 'proximity', 'connectedness', 'actionable', 'correlation', 'distance', 'sparsity', 'diversity')
            eval_results_csv.write(header)
            ...
            for x_ord in X_explain:
                ...
                MOCF_output = MOCFExplainer(x_ord, X_train, Y_train, dataset, task, predict_fn, predict_proba_fn,
                                            soundCF=True, feasibleAR=True, hof_final=False,
