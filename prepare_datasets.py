...
## Preparing Simple Binomial dataset
def PrepareSimpleBinomial(dataset_path, dataset_name, usage='causality_validation'):

    ## Creating binomial data
    def sigmoid(x):
        return 1 / (1 + np.exp(-x))
    ...
    y[y > 0.5] = 1
    y[y <= 0.5] = 0
    data = np.c_[x1,x2,x3,y]
    df = pd.DataFrame(data=data, columns=['x1', 'x2', 'x3', 'y'])
