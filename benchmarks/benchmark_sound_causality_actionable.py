...
def main():
    ...
    for dataset_kw in datsets_list:
        ...
        for blackbox_name, blackbox_constructor in blackbox_list.items():
            ...
            exists = os.path.isfile(experiment_path + 'benchmark_sound_causality_actionable_%s_cfs_%s_%s.csv'%(dataset['name'], N, n_cf))
            if exists:
                os.remove(experiment_path + 'benchmark_sound_causality_actionable_%s_cfs_%s_%s.csv'%(dataset['name'], N, n_cf))
            cfs_results_csv = open(experiment_path + 'benchmark_sound_causality_actionable_%s_cfs_%s_%s.csv'%(dataset['name'], N, n_cf), 'a')
            ...
            # creating/opening a csv file for storing results
            exists = os.path.isfile(experiment_path + 'benchmark_sound_causality_actionable_%s_eval_%s_%s.csv'%(dataset['name'], N, n_cf))
            if exists:
                os.remove(experiment_path + 'benchmark_sound_causality_actionable_%s_eval_%s_%s.csv'%(dataset['name'], N, n_cf))
            eval_results_csv = open(experiment_path + 'benchmark_sound_causality_actionable_%s_eval_%s_%s.csv'%(dataset['name'], N, n_cf), 'a')
            ...
            header = '%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n' % \
                      ('Prediction', 'Proximity', 'Connectedness', 'Causality', 'Actionable', 'Distance', 'Sparsity',
            ...
            for x_ord in X_test:
                ...
                try:
                    ...
                    DiCE_output = DiCEExplainer(x_ord, blackbox, predict_fn, predict_proba_fn, X_train, Y_train,
                                                dataset, task, CARE_output, explainer=None, actionable=True,
